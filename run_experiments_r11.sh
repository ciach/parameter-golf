...
# Round 11: phase-1 squeeze on the winning architecture.
# Focus: VE expansion, explicit late-stage QAT window, and a safe 4096 context run.
...
BATCH_NAME="${BATCH_NAME:-$(date +%Y-%m-%d_%H-%M-%S)_phase1_squeeze_winner}"
LOG_ROOT="${LOG_ROOT:-experiment_logs_r11}"
...
echo "Round 11: phase-1 squeeze on winning architecture"
...
extract_metric() {
    ...
    local metric
    metric="$(
        grep -E "final_int6_sliding_window_exact|final_int6_sliding_window_s64_exact|final_int8_zlib_roundtrip_exact" "${log_file}" 2>/dev/null \
            | tail -n 1 \
            | grep -oE 'val_bpb:[0-9.]+' \
            | cut -d: -f2
    )" || true
...
WINNER_ENV="SUBMISSION_SIZE_CAP_BYTES=16000000 EVAL_STRIDE=64 QUANT_COMPRESSOR=auto INT6_PACK=0 MIXED_QUANT_KEEP_FLOAT_MAX_NUMEL=16384 MIXED_QUANT_INT6_CATS=mlp,attn MLP_MULT=3.0 BIGRAM_VOCAB_SIZE=2048"
LATE_QAT_90_ENV="LATE_QAT_THRESHOLD=0 LATE_QAT_START_FRAC=0.90"

run_exp \
    "00_seq4096_ve_expand_late_qat" \
    "${WINNER_ENV} TRAIN_SEQ_LEN=4096 EVAL_SEQ_LEN=4096 TRAIN_BATCH_TOKENS=393216 VE_LAYERS=8,9,10 ${LATE_QAT_90_ENV}" \
    "Run this first: 4096 context + VE expansion + explicit late-QAT"

run_exp \
    "01_ve_expand_8_9_10" \
    "${WINNER_ENV} VE_LAYERS=8,9,10" \
    "Near-zero parameter VE expansion over the last three layers"

run_exp \
    "02_late_qat_last10pct" \
    "${WINNER_ENV} ${LATE_QAT_90_ENV}" \
    "Force late-QAT activation in final ~10% wallclock window"

run_exp \
    "03_ve_expand_plus_late_qat" \
    "${WINNER_ENV} VE_LAYERS=8,9,10 ${LATE_QAT_90_ENV}" \
    "Combine VE expansion with explicit late-QAT schedule"

run_exp \
    "04_seq4096_safe_batch" \
    "${WINNER_ENV} TRAIN_SEQ_LEN=4096 EVAL_SEQ_LEN=4096 TRAIN_BATCH_TOKENS=393216" \
    "4096 context probe with reduced batch tokens to mitigate memory risk"
