...
# Round 10: capacity ladder with quant-quality run first.
# Goal: use remaining byte headroom to recover BPB while staying under decimal 16MB cap.
...
BATCH_NAME="${BATCH_NAME:-$(date +%Y-%m-%d_%H-%M-%S)_capacity_ladder_quant_first}"
LOG_ROOT="${LOG_ROOT:-experiment_logs_r10}"
...
echo "Round 10: 5-run capacity ladder (quant-quality first)"
...
# User-requested order: run the former 'fifth idea' first.
run_exp \
    "00_quant_quality_tweak_mlp3_bg2048_no_embed_int6" \
    "${BASE_ENV} MIXED_QUANT_INT6_CATS=mlp,attn MLP_MULT=3.0 BIGRAM_VOCAB_SIZE=2048" \
    "Quant-quality-first: full-capacity model with embeddings kept on int8 path"

run_exp \
    "01_repeat_best_mlp2p875_bg1536" \
    "${BASE_ENV} MLP_MULT=2.875 BIGRAM_VOCAB_SIZE=1536" \
    "Repeat current best valid (variance anchor)"

run_exp \
    "02_capacity_up_mlp3_bg1536" \
    "${BASE_ENV} MLP_MULT=3.0 BIGRAM_VOCAB_SIZE=1536" \
    "Increase MLP depth while keeping bigram capacity at 1536"

run_exp \
    "03_capacity_up_plus_mlp3_bg2048" \
    "${BASE_ENV} MLP_MULT=3.0 BIGRAM_VOCAB_SIZE=2048" \
    "Restore full frontier bigram capacity with MLP 3.0"

run_exp \
    "04_no_size_arch_tweak_mlp3_bg2048_ve_8_9_10" \
    "${BASE_ENV} MLP_MULT=3.0 BIGRAM_VOCAB_SIZE=2048 VE_LAYERS=8,9,10" \
    "Near-zero-size architecture tweak: expand VE layer coverage"
